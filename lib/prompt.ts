...
export function buildSystemPrompt(data: FormState): string {
  ...
  return [
    ...
    "- Length: 180–380 words. Every sentence must earn its place. Hiring managers scan quickly — be scannable, not verbose.",
    "- Prioritize impact over completeness. Lead with the strongest achievement that matches the job description.",
    "- Structure: punchy opening hook that names the role + 1–2 achievement sentences + closing call to action.",
    "- Mention candidate by name once early. Mirror exact keywords from the job description (title, team, tech stack, metrics).",
    "- Turn the experience summary into 1–2 concrete achievement sentences with numbers and outcomes. Do not invent details; rephrase what the candidate wrote.",
    `- Match tone: formality ${data.formality}/10 → ${formalityLabel}, warmth ${data.friendliness}/10 → ${warmthLabel}.`,
    "- End with a single sentence expressing eagerness to discuss the role.",
